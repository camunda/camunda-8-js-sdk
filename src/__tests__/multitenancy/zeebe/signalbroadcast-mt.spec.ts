...
test.runIf(
	...
	'Multi-tenant Zeebe: Will throw an error if no tenantId is provided when broadcasting a signal to multi-tenant-enabled Zeebe broker',
	async () => {
...
test.runIf(
	...
	'Multi-tenant Zeebe: Will throw an error if wrong tenantId is provided when broadcasting a signal to multi-tenant-enabled Zeebe broker',
	async () => {
		const zbc = new ZeebeGrpcClient({
			config: {
...
test.runIf(
	...
	'Multi-tenant Zeebe: No error if tenantId is provided when broadcasting a signal to multi-tenant-enabled Zeebe broker',
	async () => {
		const zbc = new ZeebeGrpcClient({
			config: {
...
)('Signal broadcast to red tenant is received', async () => {
