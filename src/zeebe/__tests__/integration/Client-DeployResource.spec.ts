...
const bpmnString = fs.readFileSync(
	'./src/zeebe/__tests__/testdata/Client-DeployWorkflow.bpmn',
	'utf8'
)
...
test('deploys a DMN table from a filename', async () => {
	const result = await zbc.deployResource({
		decisionFilename: './src/zeebe/__tests__/testdata/quarantine-duration.dmn',
	})
	expect(result.deployments[0].decision.decisionKey).not.toBeNull()
})
test('deploys a DMN table', async () => {
	const decision = fs.readFileSync(
		'./src/zeebe/__tests__/testdata/quarantine-duration.dmn'
	)
	const result = await zbc.deployResource({
		decision,
...
test('deploys a Form', async () => {
	const form = fs.readFileSync('./src/zeebe/__tests__/testdata/form_1.form')
	const result = await zbc.deployResource({
		form,
		name: 'form_1.form',
	})
