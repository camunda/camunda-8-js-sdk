...
beforeAll(async () => {
	wf1 = await zbc.deployProcess(
		'./src/zeebe/__tests__/testdata/Worker-Failure1.bpmn'
	)
	bpmnProcessId1 = wf1.processes[0].bpmnProcessId
	wf2 = await zbc.deployProcess(
		'./src/zeebe/__tests__/testdata/Worker-Failure2.bpmn'
	)
	bpmnProcessId2 = wf2.processes[0].bpmnProcessId
	wf3 = await zbc.deployProcess(
		'./src/zeebe/__tests__/testdata/Worker-Failure3.bpmn'
	)
