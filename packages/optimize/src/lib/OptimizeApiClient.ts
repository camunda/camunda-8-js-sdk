...
const debug = require('debug')('optimizeapi')
...
export class OptimizeApiClient {
    ...
    private async getHeaders(auth = true) {
        const authHeader: {authorization: string} | {} = auth ? { authorization: `Bearer ${await getOptimizeToken(this.userAgentString)}` } : {} 
    ...
    async exportDashboardDefinitions(dashboardIds: string[]): Promise<Array<DashboardDefinition|SingleProcessReport>> {
        const headers = await this.getHeaders()
        return got.post(`public/export/dashboard/definition/json`, {
    ...
    async deleteReport(reportId: string) {
        const headers = await this.getHeaders()
        return got.delete(`public/report/${reportId}`, {
            headers,
            ...this.gotOptions
        }).text()
    }
